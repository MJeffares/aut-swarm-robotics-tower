...
uint16_t macAdress1 = 0;
uint16_t macAdress2 = 0;

uint64_t fullMacAdress = 0;
...
void xbeeInterpretSwarmMessage(struct MessageInfo message)
{
	...
	if(message.command == 0xE1) //Communications test
	{
		
	}	
	else if (message.command == 0xF0)
	{
		xbeeCopyData(message, dataBuffer);
		...
		if(dataBuffer[0] == 0x01)
		{
			if(dataBuffer[1] == 0x01)
			{
				...
				uint8_t sensor = dataBuffer[1];
				uint8_t DataToSend[4] = {0xF0, 0x00, sensor, LS1};
				xbeeSendAPITransmitRequest(COORDINATOR_64, UNKNOWN_16, DataToSend ,4);
				
			}
			
			if(dataBuffer[1] == 0x02)
			{
				//send a single sample
				uint8_t sensor = dataBuffer[1];
				uint8_t DataToSend[4] = {0xF0, 0x00, sensor, LS1};
		...
		else if(dataBuffer[0] == 0x02)
		{
			//start streaming every 100ms
		}
	...
	else if (message.command == 0xF1)
	{
		...
		if(dataBuffer[0] == 0x00)
		{
			...
			uint8_t messagedata = dataBuffer[1];
			
			uint8_t DataToSend[3] = {0xF0, 0x00,ALL_LEDS};

			xbeeSendAPITransmitRequest(COORDINATOR_64, UNKNOWN_16, DataToSend ,3);
		}
		else if(dataBuffer[0] == 0x01)
		{
			LEDValues = dataBuffer [1];
		}
	}
	else if (message.command == 0xF3)
	{
		xbeeCopyData(message, dataBuffer);
		
		if(dataBuffer[0] == 0x00)
		{	
			/*macAdress1 = (uint32_t)((dataBuffer[1]<<24)|(dataBuffer[2]<<16)|(dataBuffer[3]<<8)|(dataBuffer[4]<<0));
			macAdress2 = (uint32_t)((dataBuffer[5]<<24)|(dataBuffer[6]<<16)|(dataBuffer[7]<<8)|(dataBuffer[8]<<0));
			fullMacAdress = (uint64_t) ((macAdress1<<32)|(macAdress2<<0));*/			
			fullMacAdress = (uint64_t)((dataBuffer[1]<<56)|(dataBuffer[2]<<48)|(dataBuffer[3]<<40)|(dataBuffer[4]<<32)|(dataBuffer[5]<<24)|(dataBuffer[6]<<16)|(dataBuffer[7]<<8)|(dataBuffer[8]<<0));
			Task = dataBuffer[9];
			Voltage = (uint16_t)((dataBuffer[10]<<8)|(dataBuffer[11]<<0));
			Xposition = (uint16_t)((dataBuffer[12]<<8)|(dataBuffer[13]<<0));
			Yposition = (uint16_t)((dataBuffer[14]<<8)|(dataBuffer[15]<<0));	
			Facing = (int16_t)((dataBuffer[16]<<8)|(dataBuffer[17]<<0));
...
static void xbeeSendAPIFrame(uint8_t *frame_data, int len)
{
	...
	uart1Write(data[0]);				//Writes the Frame Delimiter as this shouldn't be escaped
	...
	for(int i = 1; i <data_length; i++)
	{
		...
		{
			uart1Write(ESCAPE_BYTE);	//Writes the escape byte
			uart1Write(data[i]^0x20);	//Writes the escaped byte
		}
		else
		{
			uart1Write(data[i]);	//Writes the byte
		}
	}
}
